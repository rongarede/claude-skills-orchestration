...
| 数据 | 路径 | 用途 |
|------|------|------|
| 反射定义 | `~/.claude/memory/workflows/trigger-map.md` | 当前规则完整性 |
| 统计数据 | `~/.claude/memory/workflows/trigger-stats.json` | 成功/失败/跳过计数 + CB 状态 |
| 历史运行 | `~/.claude/memory/workflows/runs/*.md` | 模式分析（重试频率、瓶颈） |
| 配置规则 | `~/.claude/CLAUDE.md` | 反射相关规则一致性 |
| Agent 记忆 | `~/.claude/memory/agents/*/` | 反射执行中的经验教训 |
...
1. 读取 `~/.claude/memory/workflows/trigger-stats.json` 全部统计
2. 扫描 `~/.claude/memory/workflows/runs/` 目录所有 workflow run 文件
